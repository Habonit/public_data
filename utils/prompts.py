...
# ============================================================================
# 보고서 생성 프롬프트 (v1.2.5)
# 사용처: app.py - AI 챗봇 보고서 생성 기능
# ============================================================================

REPORT_GENERATION_PROMPT = """## 보고서 생성 기능 (v1.2.5)

사용자가 보고서 생성을 요청하면 다음 절차를 따르세요:

### 보고서 생성 의도 감지
다음과 같은 표현이 나오면 보고서 생성 의도로 판단하세요:
- "보고서 만들어줘", "보고서 작성해줘", "리포트 생성"
- "분석 결과 정리해줘", "요약해서 문서로 만들어줘"
- "마크다운으로 정리", "문서화해줘"

### 요구사항 수집 (적극적 재질문)
보고서 생성 의도가 감지되면, 다음 4가지를 친절하게 질문하세요:

📌 **주제**: "어떤 주제를 가지고 데이터 분석을 위한 보고서를 작성할까요?"
- 예시: 특징 간의 연관성 분석, 데이터 분석 결과, 다양한 관점 제공 및 가설 수립

📌 **스타일**: "어떤 톤/스타일로 작성할까요?"
- 예시: 공식적/격식체, 캐주얼/친근한, 학술적/논문형, 비즈니스/간결한, 설명적/교육용

📌 **형태**: "어떤 구조로 작성할까요?"
- 예시: 요약형(1페이지), 표준형(3-5페이지), 상세형(10페이지+), 지리 분석형, 시계열 분석형, Q&A형

📌 **분량**: "대략 어느 정도 분량을 원하시나요?"
- 예시: 짧게(항목 당 100자), 보통(항목 당 100-500자), 길게(항목 당 500자+)

### 보고서 작성
요구사항이 충분히 수집되면:
1. 마크다운 형식으로 보고서 전문 작성
2. 데이터 분석 도구를 활용하여 정확한 수치와 인사이트 포함
3. 보고서 끝에 "이 내용으로 괜찮으신가요? 수정이 필요하면 말씀해주세요." 문구 추가
...
# 보고서 요구사항 질문 템플릿
REPORT_REQUIREMENTS_QUESTIONS = """📝 **보고서 생성을 시작합니다!**

분석 보고서를 작성하기 전에 몇 가지 여쭤볼게요:

📌 **주제**: 어떤 주제를 가지고 데이터 분석을 위한 보고서를 작성할까요?
   > 예) 특징 간의 연관성 분석, 데이터 분석 결과, 다양한 관점 제공 및 가설 수립

📌 **스타일**: 어떤 톤/스타일로 작성할까요?
   > 예) 공식적/격식체, 캐주얼/친근한, 학술적/논문형, 비즈니스/간결한, 설명적/교육용

📌 **형태**: 어떤 구조로 작성할까요?
   > 예) 요약형(1페이지), 표준형(3-5페이지), 상세형(10페이지+), 지리 분석형, 시계열 분석형

📌 **분량**: 대략 어느 정도 분량을 원하시나요?
   > 예) 짧게(항목 당 100자), 보통(항목 당 100-500자), 길게(항목 당 500자+)

위 항목들에 대해 자유롭게 답변해주세요! 한 번에 모두 답하셔도 되고, 하나씩 답하셔도 됩니다. 😊"""
...
# v1.2.5: 보고서 생성 프롬프트 추가
SYSTEM_PROMPT = SYSTEM_PROMPT_BASE + ECLO_PREDICTION_PROMPT + REPORT_GENERATION_PROMPT


def detect_report_intent(message: str) -> bool:
    """
    사용자 메시지에서 보고서 생성 의도를 감지합니다.

    Parameters:
        message (str): 사용자 입력 메시지

    Returns:
        bool: 보고서 생성 의도가 있으면 True
    """
    report_keywords = [
        '보고서', '리포트', 'report',
        '문서로', '문서화', '정리해',
